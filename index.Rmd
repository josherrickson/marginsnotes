...
While marginal effects is a generic statistical concept, Stata's
`margins` command was the first to truly support it in a
straightforward manner. On each page you'll find the Stata syntax
using the `margins` command. On other tabs, you'll find code in other
languages (R for now though others may be added) to replicate the
results. Finally, on same pages, there will be a Math tab with
additional mathematical details.

For all example, we'll use the `margex` data set from Stata.
