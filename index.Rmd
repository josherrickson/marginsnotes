...
For R, I've implemented the marginal effects estimates using both the `emmeans` and `ggeffects` packages. You'll see each in it's own tab on the right. In addition, there is an additional package, `interactions`, which we use for plotting. Finally, the `haven` package is used to read the "margex" data from Stata into R.
...
```{r child = 'R_markdown/data_pack_emmeans.Rmd'}
```

### R (ggeffects)

```{r child = 'R_markdown/data_pack_ggeffects.Rmd'}
```
